"""
Niche Medical Claims Test

Evaluates how well the MedCheck RAG system performs on highly specific,
niche medical claims compared to standard vanilla LLMs. The vanilla models
are expected to hallucinate or guess incorrectly due to lack of specific knowledge.

Queries:
1. Vanilla models (via OpenRouter)
2. MedCheck RAG API (live Render instance)

Usage: python tests/differentiators/test_niche_claims.py
"""
...
CLAIMS_FILE = Path(__file__).parent.parent / "quality" / "test_niche_claims.json"
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            if "[ACCURATE]" in chat_response:
                ...
                return "True" # Or False depending on strictness, but let's map to True 
    except Exception as e:
        pass
...
def run_niche_claims_test(limit: int | None = None) -> None:
    ...
    print(f"\n{'='*70}")
    print(f"  Niche Medical Claims Test")
    ...
    output_file = run_dir / "8_niche_claims_results.json"
    
    summary = {
        "test": "Niche Medical Claims Test",
        "total_claims": total,
        "metrics": metrics,
