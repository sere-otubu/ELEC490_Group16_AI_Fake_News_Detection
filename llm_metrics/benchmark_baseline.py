...
if not token:
    raise ValueError("Your HF_TOKEN not found in the .env file.")
login(token=token)
...
if device == "cuda":
    ...
    # Force the model to the specific GPU
    device_map = "cuda:0" 
else:
...
# Load the model with the conditional configs
model = AutoModelForCausalLM.from_pretrained(
    MODEL_ID,
    quantization_config=bnb_config, # Will be None if on CPU
...
for i, row in enumerate(test_data):
    ...
    prompt_used = "You are a medical fact-checker. Classify the following claim as 'True' or 'False' only."
...
logging.info("="*30)
logging.info(classification_report(df['truth'], df['pred'], zero_division=0))
