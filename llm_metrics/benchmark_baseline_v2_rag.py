...
log_filename = f"{LOG_DIR}/run_v2_rag_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
...
# --- SETUP RAG ---
RAG_METHOD = "Retrieval"

logging.info("Building RAG Index from Training Data...")
embedder = SentenceTransformer('all-MiniLM-L6-v2')

# Load TRAIN data to build the Knowledge Base
train_data = dataset['train']

# We will index the 'explanation' field as the knowledge source
knowledge_base = [
    f"Claim: {row['claim']} | Fact Check: {row['explanation']}" 
    for row in train_data 
    if row['explanation'] and len(str(row['explanation'])) > 10
]
...
logging.info(f"RAG Index built with {index.ntotal} documents.")
...
def get_rag_context(current_row):
    """
    Generates context based on the selected RAG method.
    """

    # RETRIEVES SIMILAR FACTS FROM TESTING DATA (Real RAG)
    query_text = current_row['claim']
    query_emb = embedder.encode([query_text], convert_to_numpy=True)
    
    # Retrieve top 3 similar past fact-checks
    D, I = index.search(query_emb, k=3)
    
    retrieved_texts = [knowledge_base[idx] for idx in I[0]]
    context_str = "\n".join([f"- {text}" for text in retrieved_texts])
    
    return f"""
    SIMILAR VERIFIED CLAIMS (CONTEXT):
    {context_str}
    """
...
for i, row in enumerate(test_data):
    ...
    rag_context = get_rag_context(row, RAG_METHOD)
    ...
    # 2. UPDATE PROMPT
    prompt_used = "You are a professional medical fact-checking assistant."
    ...
    INSTRUCTIONS:
    Based ONLY on the context above, classify this claim.
    You must output exactly one of the following words: "True", "False", "Misleading", or "Unverifiable".
    Do not explain. Just provide the label.
...
unique_filename = f"run_results/results_{model_short_name}_RAG_{run_timestamp}.csv"
...
if len(df_valid) == 0:
    ...
    logging.info("\n" + "="*30)
    logging.info(f"REPORT FOR: {MODEL_ID} with RAG")
    logging.info("="*30)
